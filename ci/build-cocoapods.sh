...
# Build for simulator
xcodebuild -configuration Release -sdk iphonesimulator CODE_SIGNING_REQUIRED=NO CODE_SIGN_IDENTITY= ONLY_ACTIVE_ARCH=NO ARCHS="i386 x86_64" BITCODE_GENERATION_MODE="bitcode" DSTROOT="$(pwd)/build/dst" OBJROOT="$(pwd)/build/obj" OTHER_CFLAGS="-fembed-bitcode" SHARED_PRECOMPS_DIR="$(pwd)/build/shared" SYMROOT="$(pwd)/build/sym" build | xcpretty -s
...
# Build for device
xcodebuild -configuration Release -sdk iphoneos CODE_SIGNING_REQUIRED=NO CODE_SIGN_IDENTITY= ONLY_ACTIVE_ARCH=NO ARCHS="arm64 armv7" BITCODE_GENERATION_MODE="bitcode" DSTROOT="$(pwd)/build/dst" OBJROOT="$(pwd)/build/obj" OTHER_CFLAGS="-fembed-bitcode" SHARED_PRECOMPS_DIR="$(pwd)/build/shared" SYMROOT="$(pwd)/build/sym" build | xcpretty -s
